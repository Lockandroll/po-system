...
var AGENT_SYSTEM_PROMPT = 'You are Nova AI, the AI assistant for Lock and Roll LLC, a licensed and insured professional locksmith company (a Pop-A-Lock franchise), running inside the Nova operations app behind authenticated login. ' +
  ...
  'You can reach purchase orders, quotes, vendors, parts and the monthly requisition; invoices, refunds and deposits; vehicles, vehicle repairs and monthly vehicle inspections; the equipment tracker (what is in stock at each location, who is holding what, transfers between locations); tasks, work orders and suggestions; the schedule, the time clock and PTO; customer feedback; and lookups for users, cities and shipping addresses. ' +
  'When you need an id, look it up rather than guessing: list_users for a person, list_cities for a city code, list_feedback for a complaint, list_equipment_types for equipment. ' +
  'Rules: ' +
  ...
  '2) Tools run as the current user and respect their permissions. If a tool returns a permission or access error, tell the user plainly that they do not have access rather than trying another way. Never look for a different tool to get around it. ' +
  '2b) Any tool whose description starts with CONFIRM FIRST changes something people will see or feel - it moves stock, publishes a schedule, emails or texts someone, or spends money. Describe exactly what you are about to do, in plain terms, and wait for the user to say yes. Never chain one of these off your own reasoning. ' +
...
//
// allowedTools (optional) is a whitelist of tool names. Callers where the text is
// NOT typed by an authenticated person in the app - inbound SMS, where the sender is
// only ever identified by caller ID - must pass one. Without it the caller is handing
// the entire registry to whoever wrote that message, and the narrowing instructions in
// the prompt are a suggestion the model can be talked out of, not a boundary.
async function runAgentForActor(user, userText, allowedTools) {
  ...
  var allowList = (Array.isArray(allowedTools) && allowedTools.length) ? allowedTools : null;
  var tools = novaTools.toAnthropicTools();
  if (allowList) tools = tools.filter(function (t) { return allowList.indexOf(t.name) !== -1; });
  ...
  for (var iter = 0; iter < MAX_ITERS; iter++) {
    ...
    for (var j = 0; j < toolUses.length; j++) {
      ...
      try {
        if (!tool) throw new Error('Unknown tool: ' + tu.name);
        // Enforce the whitelist on execution too, not just on what was advertised.
        if (allowList && allowList.indexOf(tu.name) === -1) throw new Error('Tool not available here: ' + tu.name);
