...
router.post('/:id/complete', requireAuth, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const { rows: admins } = await pool.query("SELECT email FROM users WHERE role = 'admin' AND active = true AND receive_emails = true");
      const emails = admins.map(function (a) { return a.email; }).filter(Boolean);
      if (emails.length) {
