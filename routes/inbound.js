...
router.post('/sms', async function (req, res) {
  ...
  try {
    ...
    try {
      ...
      // Caller ID is the only thing identifying this sender, and their raw message text
      // is spliced into the prompt above, so hand the agent ONLY the feedback tools it
      // needs for this record. Anything else it is asked to do, it simply cannot do.
      const FEEDBACK_SMS_TOOLS = ['get_feedback', 'update_feedback', 'resolve_feedback', 'list_users'];
      const out = await runAgentForActor(user, prompt, FEEDBACK_SMS_TOOLS);
