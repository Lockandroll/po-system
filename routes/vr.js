...
async function generateVRNumber(userInitials) {
  ...
  const { rows } = await pool.query(
    "SELECT MAX(CAST(SPLIT_PART(vr_number, '-', 3) AS INTEGER)) as maxseq FROM vehicle_repairs WHERE vr_number LIKE $1",
    [prefix]
  );
...
router.get('/', requireAuth, async function(req, res) {
  try {
    ...
    if (['admin', 'approver', 'manager'].includes(req.user.role)) {
      ...
      query = base + ' WHERE vr.vr_number IS NOT NULL AND vr.requester_id = $1' + (vehicleId ? ' AND vr.vehicle_id = $2' : '') + ' ORDER BY vr.created_at DESC';
...
router.post('/:id/submit', requireAuth, async function(req, res) {
  try {
    ...
    const { rows: approvers } = await pool.query(
      "SELECT email, name, phone, receive_emails, receive_sms FROM users WHERE role = 'admin' AND active = true"
    );
    const emailApprovers = approvers.filter(function(a) { return a.receive_emails; });
    const smsApprovers = approvers.filter(function(a) { return a.receive_sms && a.phone; });
    if (emailApprovers.length) {
      const emails = emailApprovers.map(function(a) { return a.email; });
    ...
    if (smsApprovers.length) {
      const phones = smsApprovers.map(function(a) { return a.phone; });
      await sendSms(phones, 'Lock & Roll: ' + req.user.name + ' submitted VR ' + vr.vr_number + ' for approval. Vehicle: ' + (vr.vehicle || '—') + '. Total: $' + parseFloat(vr.total_amount).toFixed(2) + '. ' + appUrl('?view=view-vr&id=' + vr.id));
    }
...
router.post('/:id/approve', requireAuth, requireRole('admin', 'approver', 'manager'), async function(req, res) {
  try {
    ...
    if (vr.requester_receive_emails !== false && vr.requester_email) {
      const html = emailTemplate({
        badge: 'Approved',
        badgeColor: 'green',
    ...
    if (vr.requester_receive_sms && vr.requester_phone) {
      await sendSms(vr.requester_phone, 'Lock & Roll: Your VR ' + vr.vr_number + ' was approved by ' + req.user.name + '. Work can now proceed. ' + appUrl('?view=view-vr&id=' + vr.id));
    }
    res.json({ success: true });
  } catch (err) {
    console.error(err);
...
router.post('/:id/reject', requireAuth, requireRole('admin', 'approver', 'manager'), async function(req, res) {
  try {
    ...
    if (vr.requester_receive_sms && vr.requester_phone) {
      await sendSms(vr.requester_phone, 'Lock & Roll: Your VR ' + vr.vr_number + ' was not approved by ' + req.user.name + (reason ? '. Reason: ' + reason : '') + '. ' + appUrl('?view=view-vr&id=' + vr.id));
    }
    res.json({ success: true });
  } catch (err) {
    console.error(err);
