...
async function initDB() {
  ...
  try {
    ...
    if (!_inspPerm.rows.length) {
      ...
      if (_rpInsp.rows.length && _rpInsp.rows[0].value) {
        try {
        ...
        } catch (e) { console.error('perm inspections backfill failed:', e.message); }
      }
      await client.query("INSERT INTO settings (key, value) VALUES ('perm_inspections_matrix_backfilled', '1') ON CONFLICT (key) DO NOTHING");
    }
    await client.query(
