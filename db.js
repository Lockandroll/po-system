...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .auth-card { width: 100%; max-width: 400px; padding: 8px; margin: 0 auto; }
    .auth-logo { text-align: center; margin-bottom: 32px; }
    .auth-logo img { display: block; margin: 0 auto 10px; height: 72px; object-fit: contain; }
    .auth-logo p { color: var(--text-muted-color); margin-top: 6px; font-size: 15px; }
...
function getSidebarSection(view) {
  if (['dashboard','new','edit','view'].indexOf(view) !== -1) return 'po';
...
async function render() {
  ...
  var poViews = ['dashboard','new','edit','view'];
...
function renderLogin(app) {
  app.innerHTML =
    '<div class="auth-page"><div class="auth-card">' +
      '<div class="auth-logo"><div style="background:#fff;border-radius:14px;padding:14px 28px;display:inline-block;margin-bottom:12px;"><img src="https://www.popalock.com/wp-content/uploads/2020/11/pal-logo-highres.png" alt="Pop-A-Lock" /></div><p>Sign in to your account</p></div>' +
...
function renderSetup(app) {
  app.innerHTML =
    '<div class="auth-page"><div class="auth-card">' +
      '<div class="auth-logo"><div style="background:#fff;border-radius:14px;padding:14px 28px;display:inline-block;margin-bottom:12px;"><img src="https://www.popalock.com/wp-content/uploads/2020/11/pal-logo-highres.png" alt="Pop-A-Lock" /></div><p>Create your admin account</p></div>' +
      '<div class="card" style="background:transparent;border:1px solid rgba(255,255,255,0.18);"><div class="card-body">' +
        '<div id="auth-error"></div>' +
...
async function renderViewPO(el, id) {
  ...
  try {
    ...
    const canEdit = state.user.role === 'admin' || (state.user.role !== 'approver' && po.status === 'draft' && po.requester_id === state.user.id);
    const canSubmit = state.user.role !== 'approver' && po.status === 'draft' && po.requester_id === state.user.id;
    ...
    el.innerHTML =
      '<div class="page-header">' +
        ...
        '<div class="flex-gap">' +
          ...
          (canSubmit ? '<button class="btn btn-primary" onclick="submitPO(' + po.id + ')">Submit for Approval</button>' : '') +
    ...
    function vrDashRender(q, statusFilter) {
      ...
      tbody.innerHTML = filtered.length ? filtered.map(function(vr) {
        ...
        return '<tr style="cursor:pointer" onclick="navigate(\'view-vr\',' + vr.id + ')">' +
          '<td><strong style="color:var(--primary)">' + escHtml(vr.vr_number) + '</strong></td>' +
