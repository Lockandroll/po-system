...
async function requireAuth(req, res, next) {
  ...
  // A hard-deleted account leaves no row at all. Every DB gate below is written as
  // "if (urow && ...)", so a missing row used to skip the active check, the session
  // check and the offboarding check, and fall back to the role claimed in the token.
  // Deleting a user is meant to end their access, so treat a missing row as 401.
  if (!urow && !payload.addin) {
    return res.status(401).json({ error: 'Your account no longer exists.', deactivated: true });
  }
  ...
  // The gate keys off the token's onb claim. Nova AI tools call back into this API with
  // a freshly minted token (lib/novaTools.js) that carries an nt flag instead, so the
  // check has to run for those too - otherwise a hire mid-onboarding could reach the
  // whole API through the assistant. Either way the status itself comes from the DB.
  let onbActive = false;
  if (payload.onb || payload.nt) {
    if (userDbErr) {
  ...
  if (viewAs && req.user.role === 'admin') {
    ...
    try {
      ...
      if (r.rows.length) {
        const t = r.rows[0];
        // Compare against the role in the DATABASE, not the token: someone demoted from
        // owner still holds a token claiming owner until it expires.
        if (!(t.role === 'owner' && effRole !== 'owner')) {
