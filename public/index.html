...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #sidebar { width: 240px; background: #111; border-right: 1px solid var(--border); display: flex; flex-direction: column; position: fixed; top: 0; left: 0; height: 100vh; z-index: 100; transition: transform 0.25s ease; }
    #main { margin-left: 240px; flex: 1; display: flex; flex-direction: column; min-height: 100vh; }
    .main-header { background: #111; border-bottom: 1px solid var(--border); padding: 16px 24px; display: flex; align-items: center; justify-content: space-between; }
    .main-content { padding: 24px; flex: 1; }
    #app.no-sidebar #sidebar { display: none; }
    #app.no-sidebar #main { margin-left: 0; }
    ...
    .sidebar-logo h1 { font-size: 18px; font-weight: 700; color: var(--primary); letter-spacing: -0.3px; }
    .sidebar-logo h1::after { content: ''; display: block; width: 32px; height: 2px; background: var(--primary); margin-top: 6px; border-radius: 2px; }
    .sidebar-logo p { font-size: 12px; color: var(--text-muted-color); margin-top: 2px; }
    .sidebar-nav { flex: 1; padding: 12px 8px; }
    .nav-item { display: flex; align-items: center; gap: 10px; padding: 10px 12px; border-radius: var(--radius); cursor: pointer; color: var(--text-dim); font-size: 14px; font-weight: 500; transition: all 0.15s; margin-bottom: 2px; }
    .nav-item:hover { background: var(--bg-elevated); color: var(--text); }
    .nav-item.active { background: rgba(249,115,22,0.15); color: var(--primary); }
    .nav-item svg { width: 18px; height: 18px; flex-shrink: 0; }
    .nav-section-header { display:flex; align-items:center; justify-content:space-between; padding:10px 12px; border-radius:var(--radius); cursor:pointer; color:var(--text-dim); font-size:14px; font-weight:500; transition:all 0.15s; user-select:none; margin-bottom:2px; }
    .nav-section-header:hover { background:var(--bg-elevated); color:var(--text); }
    .nav-section-header.open { background:var(--bg-elevated); color:var(--text); }
    .nav-section-header.section-active { color:var(--primary); }
    .nav-section-header .s-label { display:flex; align-items:center; gap:10px; pointer-events:none; }
    .nav-section-header .s-label svg { width:18px; height:18px; flex-shrink:0; }
    .nav-chevron { opacity:0.4; transition:transform 0.2s ease; flex-shrink:0; pointer-events:none; }
    .nav-section-header.open .nav-chevron { transform:rotate(90deg); opacity:0.7; }
    .nav-sub { display:flex; align-items:center; gap:8px; padding:7px 12px 7px 36px; border-radius:var(--radius); cursor:pointer; color:var(--text-muted-color); font-size:13px; transition:all 0.15s; margin-bottom:1px; }
    .nav-sub:hover { background:var(--bg-elevated); color:var(--text); }
    .nav-sub.active { background:rgba(249,115,22,0.12); color:var(--primary); font-weight:600; }
    .nav-sub svg { width:14px; height:14px; flex-shrink:0; }
    .sidebar-footer { padding: 16px; border-top: 1px solid var(--border); }
    .sidebar-user { display: flex; align-items: center; gap: 10px; margin-bottom: 12px; }
    .avatar { width: 36px; height: 36px; border-radius: 50%; background: var(--primary); color: white; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 14px; flex-shrink: 0; }
    .sidebar-user-info { flex: 1; min-width: 0; }
    .sidebar-user-name { font-size: 14px; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .sidebar-user-role { font-size: 12px; color: var(--text-muted-color); text-transform: capitalize; }
    .btn { display: inline-flex; align-items: center; gap: 6px; padding: 8px 16px; border-radius: var(--radius); font-size: 14px; font-weight: 500; cursor: pointer; border: none; transition: all 0.15s; text-decoration: none; }
    ...
    .btn-ghost { background: transparent; color: var(--text-muted-color); }
    .btn-ghost:hover { background: var(--bg-elevated); color: var(--text-dim); }
    ...
    label { display: block; font-size: 14px; font-weight: 500; color: var(--text-dim); margin-bottom: 6px; }
    input, select, textarea { width: 100%; padding: 9px 12px; border: 1px solid var(--border); border-radius: var(--radius); font-size: 14px; color: var(--text); background: var(--bg-elevated); transition: border 0.15s; outline: none; }
    input:focus, select:focus, textarea:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(249,115,22,0.15); }
    select option { background: var(--bg-elevated); color: var(--text); }
    textarea { resize: vertical; min-height: 80px; }
    .form-row { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
    .table-wrap { overflow-x: auto; }
    table { width: 100%; border-collapse: collapse; }
    th { padding: 10px 16px; text-align: left; font-size: 12px; font-weight: 600; color: var(--text-muted-color); text-transform: uppercase; letter-spacing: 0.05em; border-bottom: 1px solid var(--border); background: var(--bg-elevated); }
    td { padding: 12px 16px; font-size: 14px; border-bottom: 1px solid var(--border-light); color: var(--text-dim); }
    ...
    .badge-draft { background: #2a2a2a; color: #aaa; }
    .badge-submitted { background: #2d2100; color: #f59e0b; border: 1px solid #4a3500; }
    .badge-approved { background: #0d2d17; color: #22c55e; border: 1px solid #134d27; }
    .badge-rejected { background: #2d0d0d; color: #ef4444; border: 1px solid #4d1515; }
    .badge-cancelled { background: #1a0d2e; color: #c084fc; border: 1px solid #3b1f6e; }
    .badge-order-placed { background: #1a0d2e; color: #a78bfa; border: 1px solid #3b1f6e; }
    .badge-admin { background: #1e1040; color: #a78bfa; }
    .badge-approver { background: #0d1e30; color: #60a5fa; }
    .badge-requester { background: #2a2a2a; color: #aaa; }
    .badge-inactive { background: #1a1a1a; color: #555; border: 1px solid #2a2a2a; }
    .user-row-inactive td { opacity: 0.5; }
    .page-header { margin-bottom: 24px; display: flex; align-items: center; justify-content: space-between; }
    .page-title { font-size: 22px; font-weight: 700; }
    .page-subtitle { font-size: 14px; color: var(--text-muted-color); margin-top: 4px; }
    ...
    .stat-value { font-size: 28px; font-weight: 700; margin-bottom: 4px; color: var(--primary); }
    .stat-label { font-size: 13px; color: var(--text-muted-color); }
    .line-items-table { width: 100%; border-collapse: collapse; margin-top: 8px; }
    .line-items-table th { padding: 8px 10px; background: var(--bg-elevated); font-size: 12px; font-weight: 600; color: var(--text-muted-color); text-transform: uppercase; border-bottom: 1px solid var(--border); text-align: left; }
    .line-items-table td { padding: 8px 10px; font-size: 14px; border-bottom: 1px solid var(--border-light); color: var(--text-dim); }
    .line-items-table td input { padding: 6px 8px; }
    .line-items-table td .btn svg { width: 16px; height: 16px; flex-shrink: 0; }
    .line-items-table .total-row td { font-weight: 700; background: var(--bg-elevated); color: var(--primary); }
    .add-item-btn { margin-top: 8px; }
    .detail-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 24px; }
    .detail-field label { font-size: 12px; font-weight: 600; color: var(--text-muted-color); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 4px; }
    .detail-field p { font-size: 15px; color: var(--text); }
    .alert { padding: 12px 16px; border-radius: var(--radius); font-size: 14px; margin-bottom: 16px; }
    .alert-error { background: #2d0d0d; color: #fca5a5; border: 1px solid #4d1515; }
    .alert-success { background: #0d2d17; color: #86efac; border: 1px solid #134d27; }
    .alert-info { background: #0d1e30; color: #93c5fd; border: 1px solid #1e3a5f; }
    .auth-page { display: flex; align-items: center; justify-content: center; min-height: 100vh; background: var(--bg); width: 100%; }
    .auth-card { width: 100%; max-width: 400px; padding: 8px; margin: 0 auto; text-align: center; }
    .auth-logo { text-align: center; margin-bottom: 28px; }
    .auth-logo .logo-wrap { border: 1px solid rgba(255,255,255,0.35); border-radius: 10px; padding: 8px 18px; display: inline-block; margin-bottom: 12px; }
    .auth-logo img { display: block; height: 72px; object-fit: contain; }
    .auth-logo h2 { font-size: 24px; font-weight: 800; color: var(--primary); margin-bottom: 4px; }
    .auth-logo p { color: var(--text-muted-color); font-size: 14px; }
    ...
    .modal { background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius); box-shadow: var(--shadow-md); width: 100%; max-width: 480px; }
    ...
    .empty-state { text-align: center; padding: 48px 24px; color: var(--text-muted-color); }
    ...
    .loading { text-align: center; padding: 48px; color: var(--text-muted-color); }
    .flex-gap { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; }
    .text-right { text-align: right; }
    .text-muted { color: var(--text-muted-color); font-size: 13px; }
    .mt-2 { margin-top: 8px; }
...
<body>
<canvas id="ai-canvas-left" style="position:fixed;top:0;left:0;pointer-events:none;z-index:0"></canvas>
<div id="app" class="no-sidebar"></div>
<script>
const state = {
  token: localStorage.getItem('po_token'),
  user: JSON.parse(localStorage.getItem('po_user') || 'null'),
  currentView: null,
  currentParam: null,
  sidebarSection: null,
  pendingUserId: null,
...
function getSidebarSection(view) {
  ...
  if (['company-info','ai-context','settings','users','cities','audit'].indexOf(view) !== -1) return 'settings';
  return null;
}
function toggleSection(section, defaultView) {
  if (state.sidebarSection === section && !defaultView) {
    state.sidebarSection = null; render(); return;
  }
...
async function render() {
  ...
  var stViews = ['company-info','ai-context','settings','users','cities','audit'];
  var navHtml =
    ...
    (ss === 'po' ?
      ...
      (state.user.role !== 'approver' ? '<div class="nav-sub' + (cv === 'new' ? ' active' : '') + '" onclick="navigate(\'new\')">' + icons.plus + ' New PO</div>' : '') +
      (state.user.role !== 'approver' ? '<div class="nav-sub' + (cv === 'running' || cv === 'running-admin' ? ' active' : '') + '" onclick="navigate(\'' + (isAdminMgr ? 'running-admin' : 'running') + '\')"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg> Monthly Req</div>' : '')
    : '') +
    ...
    (ss === 'quotes' ?
      ...
      (state.user.role !== 'approver' ? '<div class="nav-sub' + (cv === 'new-quote' ? ' active' : '') + '" onclick="navigate(\'new-quote\')">' + icons.plus + ' New Quote</div>' : '')
    : '') +
    ...
    (ss === 'vr' ?
      ...
      (state.user.role !== 'approver' ? '<div class="nav-sub' + (cv === 'new-vr' ? ' active' : '') + '" onclick="navigate(\'new-vr\')">' + icons.plus + ' New VR</div>' : '') +
      (isAdminMgr ? '<div class="nav-sub' + (['fleet-registry','new-vehicle','edit-vehicle','vehicle-history'].indexOf(cv) !== -1 ? ' active' : '') + '" onclick="navigate(\'fleet-registry\')">' + icoDB + ' Fleet Registry</div>' : '')
    : '') +
    (isAdminMgr ? '<div class="nav-item' + (cv === 'vendors' ? ' active' : '') + '" onclick="navigate(\'vendors\')">' + icoAccounts + ' Accounts</div>' : '') +
    '<div class="nav-item' + (cv === 'suggestions' ? ' active' : '') + '" onclick="navigate(\'suggestions\')">' + icoSuggestion + ' Suggestions</div>' +
    (isAdmin ?
      '<div class="nav-section-header' + (aiViews.indexOf(cv) !== -1 ? ' section-active' : '') + (ss === 'ai' ? ' open' : '') + '" onclick="toggleSection(\'ai\',\'ai-assistant\')"><span class="s-label">' + icoAI + ' Locksmith AI Assistant</span>' + chev + '</div>' +
      (ss === 'ai' ?
      ...
      : '')
    :
      '<div class="nav-item' + (cv === 'ai-assistant' ? ' active' : '') + '" onclick="navigate(\'ai-assistant\')">' + icoAI + ' Locksmith AI Assistant</div>'
    ) +
    (isAdminMgr ?
      ...
      (ss === 'settings' ?
        '<div class="nav-sub' + (cv === 'company-info' || cv === 'settings' ? ' active' : '') + '" onclick="navigate(\'company-info\')">' + icoBuilding + ' Company Information</div>' +
        (isAdmin ? '<div class="nav-sub' + (cv === 'ai-context' ? ' active' : '') + '" onclick="navigate(\'ai-context\')">' + icoAI + ' AI Context</div>' : '') +
        '<div class="nav-sub' + (cv === 'users' ? ' active' : '') + '" onclick="navigate(\'users\')">' + icons.users + ' Users</div>' +
        '<div class="nav-sub' + (cv === 'cities' ? ' active' : '') + '" onclick="navigate(\'cities\')">' + icons.map + ' Cities</div>' +
        (isAdmin ? '<div class="nav-sub' + (cv === 'audit' ? ' active' : '') + '" onclick="navigate(\'audit\')">' + icoAudit + ' Audit Log</div>' : '')
      : '')
    : '');
  app.innerHTML =
    '<div id="sidebar">' +
      ...
      '<div class="sidebar-footer">' +
        '<div class="sidebar-user"><div class="avatar">' + escHtml(initials) + '</div><div class="sidebar-user-info"><div class="sidebar-user-name">' + escHtml(state.user.name) + '</div><div class="sidebar-user-role">' + escHtml(state.user.role) + '</div></div></div>' +
...
async function doVerify2FA() {
  ...
  try {
    const data = await api('POST', '/auth/verify-2fa', { userId: state.pendingUserId, code: code });
...
async function renderDashboard(el) {
  try {
    ...
    const isApprover = ['approver','admin'].includes(state.user.role);
    ...
    el.innerHTML =
      ...
      (state.user.role !== 'approver' ? '<button class="btn btn-primary" onclick="navigate(\'new\')" style="white-space:nowrap">' + icons.plus + ' New PO</button>' : '') +
      '</div></div>' +
...
function renderPagination(page, totalPages, totalItems, callbackName) {
  ...
  for (let i = 1; i <= totalPages; i++) {
    if (i === 1 || i === totalPages || Math.abs(i - page) <= 1) {
      ...
      btns += '<span style="color:var(--text-muted-color);padding:0 2px">…</span>';
    }
  }
  return '<div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-top:1px solid var(--border)">' +
    '<span style="font-size:13px;color:var(--text-muted-color)">Showing ' + start + '–' + end + ' of ' + totalItems + '</span>' +
    '<div style="display:flex;gap:4px;align-items:center">' +
      '<button onclick="' + callbackName + '(' + (page - 1) + ')" ' + (page === 1 ? 'disabled' : '') + ' style="padding:4px 8px;border-radius:4px;border:1px solid var(--border);background:var(--bg-elevated);color:var(--text-dim);cursor:pointer;font-size:13px">‹</button>' +
      btns +
      '<button onclick="' + callbackName + '(' + (page + 1) + ')" ' + (page === totalPages ? 'disabled' : '') + ' style="padding:4px 8px;border-radius:4px;border:1px solid var(--border);background:var(--bg-elevated);color:var(--text-dim);cursor:pointer;font-size:13px">›</button>' +
    '</div>' +
  '</div>';
}
...
function applyFilters(resetPage) {
  ...
  tbody.innerHTML =
    ...
    '<thead><tr><th>PO Number</th><th>City</th><th>Vendor</th><th>Customer/Employee</th>' + (isApprover ? '<th>Requested By</th>' : '') + '<th>Total</th><th>Status</th><th>Approved By</th><th>Orderer</th><th>Date</th>' + (state.user.role === 'admin' || state.user.role === 'approver' ? '<th></th>' : '') + '</tr></thead>' +
    '<tbody>' + page.map(function(po){
      return '<tr style="cursor:pointer" onclick="navigate(\'view\',' + po.id + ')">' +
        ...
        ((state.user.role === 'admin' || state.user.role === 'approver') && po.status === 'approved' && (state.user.role === 'admin' || po.orderer_id === state.user.id)
          ...
          : (state.user.role === 'admin' || state.user.role === 'approver' ? '<td></td>' : '')) +
      '</tr>';
    }).join('') +
    '</tbody></table></div>' +
    renderPagination(_poPage, totalPages, filtered.length, 'poPaginate');
}
...
async function renderRunningList(el){
  ...
  try { runningCities = await api('GET','/cities'); } catch(e){ runningCities = []; }
  try { runningItems = await api('GET','/running'); } catch(e){ el.innerHTML = '<div class="alert alert-error">' + escHtml(e.message) + '</div>'; return; }
  runningCityForNew = (runningItems[0] && runningItems[0].city_code) || (runningCities[0] && runningCities[0].code) || '';
...
function adminRunRenderShell(el, cities){
  ...
  el.innerHTML =
    ...
    '<div class="card mb-4"><div class="card-body">' +
      ...
      '</div>' +
      '<div class="table-wrap"><table class="line-items-table" style="table-layout:fixed">' +
        '<colgroup><col style="width:6%"><col style="width:23%"><col style="width:13%"><col style="width:8%"><col style="width:12%"><col style="width:23%"><col style="width:15%"></colgroup>' +
        '<thead><tr><th></th><th>Item</th><th>Requester</th><th>Qty</th><th>Est. Cost</th><th>Vendor</th><th>Part Description</th></tr></thead>' +
        '<tbody id="admin-run-body"></tbody>' +
        '<tfoot><tr class="total-row"><td colspan="4" class="text-right" style="padding:10px">Est. Total</td><td id="admin-run-total" style="padding:10px">$0.00</td><td colspan="2"></td></tr></tfoot>' +
      '</table></div>' +
...
function adminRunBuildRows(){
  ...
  if(!items.length){
    tbody.innerHTML = '<tr><td colspan="7" style="padding:14px;color:var(--text-muted-color)">No items for this city.</td></tr>';
...
async function renderViewPO(el, id) {
  ...
  try {
    ...
    const canEdit = state.user.role === 'admin' || (state.user.role !== 'approver' && (po.status === 'draft' || po.status === 'rejected') && po.requester_id === state.user.id);
    const canSubmit = state.user.role !== 'approver' && (po.status === 'draft' || po.status === 'rejected') && po.requester_id === state.user.id;
    const canDelete = state.user.role === 'admin' || (state.user.role !== 'approver' && po.status === 'draft' && po.requester_id === state.user.id);
    const canApprove = ['approver','admin'].includes(state.user.role) && po.status === 'submitted';
    const canOrder = po.status === 'approved' && (state.user.role === 'admin' || po.orderer_id === state.user.id);
    const canCancel = state.user.role === 'admin' && po.status !== 'draft' && po.status !== 'cancelled';
...
async function renderUsers(el) {
  ...
  try {
    ...
    _usersSearch = '';
    el.innerHTML =
      '<div class="page-header"><div class="page-title">Users</div>' + (state.user.role === 'admin' ? '<button class="btn btn-primary" onclick="showUserModal(null)" style="white-space:nowrap">' + icons.plus + ' Add User</button>' : '') + '</div>' +
...
function usersRenderTable() {
  ...
  wrap.innerHTML =
    '<div class="card"><div class="card-body" style="padding:0"><div class="table-wrap"><table>' +
      '<thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Status</th>' + (state.user.role === 'admin' ? '<th></th>' : '') + '</tr></thead>' +
      '<tbody>' + (page.length ? page.map(function(u) {
        ...
        return '<tr class="' + (isInactive ? 'user-row-inactive' : '') + '">' +
          ...
          (state.user.role === 'admin' ? '<td class="flex-gap">' +
          ...
          '</td>' : '') +
        '</tr>';
      }).join('') : '<tr><td colspan="5" style="text-align:center;color:var(--text-muted-color);padding:24px">No users found</td></tr>') +
      '</tbody></table></div></div></div>';

  // Pagination controls
  var pg = document.getElementById('users-pagination');
  if (!pg) return;
  if (totalPages <= 1) { pg.innerHTML = '<span style="font-size:13px;color:var(--text-muted-color)">' + filtered.length + ' user' + (filtered.length !== 1 ? 's' : '') + '</span>'; return; }
  var html = '<span style="font-size:13px;color:var(--text-muted-color);margin-right:4px">' + filtered.length + ' users</span>';
...
async function showUserModal(id) {
  let user = null;
  if (id) { try { const users = await api('GET', '/users'); user = users.find(function(u){ return u.id === id; }); } catch(e){} }
  ...
  overlay.innerHTML =
    '<div class="modal">' +
      ...
      '<div class="modal-body">' +
        ...
        '<div class="form-group"><label>' + (id ? 'New Password (leave blank to keep)' : 'Password') + '</label><input type="password" id="modal-password" /></div>' +
        '<div class="form-row">' +
          ...
          '<div class="form-group"><label>Role</label><select id="modal-role"><option value="requester"' + (user&&user.role==='requester'?' selected':'') + '>Requester</option><option value="approver"' + (user&&user.role==='approver'?' selected':'') + '>Approver</option><option value="admin"' + (user&&user.role==='admin'?' selected':'') + '>Admin</option><option value="manager"' + (user&&user.role==='manager'?' selected':'') + '>Manager</option></select></div>' +
          '</div>' +
...
async function saveUser(id, btn) {
  ...
  try {
    btn.disabled = true;
    if (id) { await api('PUT', '/users/' + id, { name, email, password: password || undefined, role, phone: phone || undefined, receive_emails, receive_sms }); }
    else { await api('POST', '/users', { name, email, password, role, phone: phone || undefined, receive_emails, receive_sms }); }
...
function vendorsRenderTable(search) {
  ...
  wrap.innerHTML =
    '<div class="card"><div class="table-wrap">' +
      '<table>' +
        '<thead><tr><th>Account Name</th><th>Website</th><th>Account #</th><th>Username</th><th>Password</th><th>Notes</th><th>Rep Name</th><th>Rep Email</th><th>Rep Phone</th><th></th></tr></thead>' +
        '<tbody>' +
          (filtered.length === 0
            ? '<tr><td colspan="10" style="text-align:center;color:var(--text-muted-color);padding:32px">No accounts found.</td></tr>'
...
function showVendorModal(id, name, website, account_number, username, password, notes, rep_name, rep_email, rep_phone) {
  const isEdit = !!id;
  const overlay = document.createElement('div');
  overlay.className = 'modal-overlay';
  ...
  try {
    ...
    el.innerHTML =
      '<div class="page-header">' +
        ...
        '<button class="btn btn-primary" onclick="navigate(\'new-quote\')" style="white-space:nowrap">' + icons.plus + ' New Quote</button>' +
      '</div>' +
...
function filterQuotes(resetPage) {
  ...
  wrap.innerHTML =
    ...
    '</table></div>' +
    renderPagination(_quotePage, totalPages, filtered.length, 'quotePaginate');
}
...
async function renderViewQuote(el, id) {
  ...
  try {
    ...
    _currentQuote = q;
    const canEdit = state.user.role === 'admin' || q.requester_id === state.user.id;
    const canDelete = state.user.role === 'admin' || q.requester_id === state.user.id;
    el.innerHTML =
      '<div class="page-header">' +
        ...
        '<div class="flex-gap">' +
          ...
          '<button class="btn btn-secondary" onclick="printQuote(' + q.id + ')">' + icons.print + ' Print Quote</button>' +
          ...
          (canDelete ? '<button class="btn btn-danger btn-sm" onclick="deleteQuote(' + q.id + ')">' + icons.trash + '</button>' : '') +
        '</div>' +
      '</div>' +
...
async function renderAuditLog(el) {
  ...
  try {
    ...
    _auditPage = 1;
    el.innerHTML =
      '<div class="page-header"><div><div class="page-title">Audit Log</div><div class="page-subtitle">All PO and Quote activity</div></div>' +
      '<div class="flex-gap">' +
        '<select id="audit-filter-type" onchange="auditApplyFilter()" style="width:auto"><option value="">All Types</option><option value="po">POs Only</option><option value="quote">Quotes Only</option></select>' +
        '<select id="audit-filter-action" onchange="auditApplyFilter()" style="width:auto"><option value="">All Actions</option><option value="created">Created</option><option value="edited">Edited</option><option value="submitted">Submitted</option><option value="approved">Approved</option><option value="rejected">Rejected</option><option value="cancelled">Cancelled</option><option value="deleted">Deleted</option></select>' +
      '</div></div>' +
...
function auditRenderTable() {
  ...
  if (!pg) return;
  if (totalPages <= 1) { pg.innerHTML = '<span style="font-size:13px;color:var(--text-muted-color)">' + filtered.length + ' record' + (filtered.length !== 1 ? 's' : '') + '</span>'; return; }
  var html = '<span style="font-size:13px;color:var(--text-muted-color);margin-right:4px">' + filtered.length + ' records</span>';
...
function renderAuditRows(logs) {
  const actionColors = { created: '#22c55e', edited: '#60a5fa', deleted: '#ef4444', submitted: '#f59e0b', approved: '#22c55e', rejected: '#ef4444', cancelled: '#c084fc' };
...
function aiConvRenderPage() {
  ...
  if (!pg) return;
  if (totalPages <= 1) { pg.innerHTML = '<span style="font-size:13px;color:var(--text-muted-color)">' + filtered.length + ' conversation' + (filtered.length !== 1 ? 's' : '') + '</span>'; return; }
  var html = '<span style="font-size:13px;color:var(--text-muted-color);margin-right:4px">' + filtered.length + ' conversations</span>';
...
async function renderAIAssistant(el) {
  ...
  el.innerHTML =
    ...
    '</div>' +
    '<svg class="ai-side-icon-left" width="160" height="160" viewBox="0 0 24 24" fill="currentColor"><path d="M12 1C9.24 1 7 3.24 7 6v2H5a2 2 0 00-2 2v11a2 2 0 002 2h14a2 2 0 002-2V10a2 2 0 00-2-2h-2V6c0-2.76-2.24-5-5-5zm0 2c1.66 0 3 1.34 3 4v2H9V6c0-1.66 1.34-3 3-3zm0 9a2 2 0 110 4 2 2 0 010-4z"/></svg>' +
    '<svg class="ai-side-icon-right" width="160" height="160" viewBox="0 0 24 24" fill="currentColor"><path d="M12 1C9.24 1 7 3.24 7 6v2H5a2 2 0 00-2 2v11a2 2 0 002 2h14a2 2 0 002-2V10a2 2 0 00-2-2h-2V6c0-2.76-2.24-5-5-5zm0 2c1.66 0 3 1.34 3 4v2H9V6c0-1.66 1.34-3 3-3zm0 9a2 2 0 110 4 2 2 0 010-4z"/></svg>' +
...
async function renderVRDashboard(el) {
  ...
  try {
    ...
    const isApprover = ['admin','approver','manager'].includes(state.user.role);
    ...
    el.innerHTML =
      '<div class="page-header">' +
        ...
        (state.user.role !== 'approver' ? '<button class="btn btn-primary" onclick="navigate(\'new-vr\')" style="white-space:nowrap">' + icons.plus + ' New VR</button>' : '') +
      '</div>' +
...
async function renderViewVR(el, id) {
  ...
  try {
    ...
    var isApprover = ['admin','approver','manager'].includes(state.user.role);
    ...
    el.innerHTML =
      '<div class="page-header">' +
        ...
        '</div>' +
        '<div class="flex-gap">' +
          (vr.status==='draft' && (isOwner||state.user.role==='admin') ? '<button class="btn btn-secondary" onclick="navigate(\'edit-vr\',' + vr.id + ')">Edit</button>' : '') +
          (vr.status==='draft' && (isOwner||state.user.role==='admin') ? '<button class="btn btn-primary" onclick="submitVR(' + vr.id + ')">Submit for Approval</button>' : '') +
          ...
          ((vr.status==='draft' && (isOwner||state.user.role==='admin')) || state.user.role==='admin' ? '<button class="btn btn-danger btn-sm" onclick="deleteVR(' + vr.id + ')">Delete</button>' : '') +
...
function applyFleetFilters(resetPage) {
  ...
  document.getElementById('fleet-pagination').innerHTML = renderPagination(_fleetPage, totalPages, filtered.length, 'fleetPaginate');
}
  ...
  try {
    ...
    el.innerHTML =
      ...
      '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:24px">' +
        (isPrivileged ? '<div class="card" style="cursor:pointer" onclick="navigate(\'vr-dashboard\')"><div class="card-body" style="text-align:center;padding:16px">' +
        ...
        '<div class="card" style="cursor:pointer" onclick="navigate(\'dashboard\')"><div class="card-body" style="text-align:center;padding:16px">' +
        ...
        '<div class="card" style="cursor:pointer" onclick="navigate(\'quotes\')"><div class="card-body" style="text-align:center;padding:16px">' +
      ...
      '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px">' +
        (state.user.role !== 'approver' ? '<button class="btn btn-secondary" onclick="navigate(\'new\')" style="padding:14px;display:flex;flex-direction:column;align-items:center;gap:6px;height:auto">' +
        ...
        (state.user.role !== 'approver' ? '<button class="btn btn-secondary" onclick="navigate(\'new-quote\')" style="padding:14px;display:flex;flex-direction:column;align-items:center;gap:6px;height:auto">' +
        ...
        (state.user.role !== 'approver' ? '<button class="btn btn-secondary" onclick="navigate(\'new-vr\')" style="padding:14px;display:flex;flex-direction:column;align-items:center;gap:6px;height:auto">' +
    ...
    el.innerHTML =
      '<div style="text-align:center;padding:48px 20px;">' +
        ...
        '<div style="color:var(--text-muted-color);font-size:14px;margin-bottom:20px;">This sometimes happens after a fresh deploy. Try refreshing the page.</div>' +
...
async function loadSuggestionsTable() {
  ...
  if (!area) return;
  try {
    var data = await api('GET', '/suggestions');
    if (!data.length) {
...
@media (min-width:1400px) {
  ...
  .ai-side-icon-left { left:calc(260px + 40px); }
  .ai-side-icon-right { right:40px; }
}
.ai-side-icon-left,
.ai-side-icon-right { display:none; }
</style>
</body>
</html>
